...
void loop(){
  if(Serial.available()){                                       //Seri haberleşme verisi geldiğinde içindeki komutlar çalışır.
    if(Serial.find("+IPD,")){                                   //Bağlantı isteği geldiğinde içindeki komutlar çalışır.
      ...
      String sayfa = "<h1>Step Motor Kontrol</h1><br>";       //html kodları bu satırlar                    
...
String espOkuma(long int zamanAsimi){                           //ESP'den gelen dönütleri okur.
  long int baslangic = millis();//Arduino ya enerji verdiğimiz zaman arduino içerisindeki sayaçtan alınan değerdir(milisaniye cinsinden sayar siz o anki değeri öğrenebilrisiniz bununla)
  ...
  gelen.replace("AT+","");//AT yazan yerlere boşluk koyacak böylelikle sadece ip ve mac adresmizi görmüş olacağız
  return gelen;
}
void serialTemizle(long int zamanAsimi){                         //Seri haberleşmede kullanmadığımız byteları temizler.
  ...
  while(millis() - baslangic < zamanAsimi){
    if(Serial.available()>0){
        Serial.read(); //burdaki bytlari temizleme işlerini gerçekleştirmiş oluyoruz
    } 
  }
}
